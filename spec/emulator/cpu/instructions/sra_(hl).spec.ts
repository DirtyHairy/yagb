...
describe('The glorious CPU', () => {
    describe('SRA (HL)', () => {
        function setup(value: number, carry: boolean): Environment {
            const env = newEnvironment([0xcb, 0x2e]);
        ...
        it('Shift (HL) right', () => {
            ...
            expect(bus.read(cpu.state.r16[r16.hl])).toBe(0xd2);
        });
        ...
        describe('unaffected Bit 7', () => {
            it('Bit 7 set', () => {
                ...
                expect(bus.read(cpu.state.r16[r16.hl]) & 0x80).toBe(0x80);
            });
            it('Bit 7 not set', () => {
                const { bus, cpu } = setup(0x25, true);

                cpu.step(1);

                expect(bus.read(cpu.state.r16[r16.hl]) & 0x80).toBe(0x0);
            });
        });
